...
library(gujarati); library(tidyverse); library(zoo); library(forecast); 
...
# La serie de tiempo del PIB crece a lo largo de todo el período, es decir que 
# muestra una tendencia ascendente y por lo tanto no es estacionaria

# Correlograma de la serie
ggAcf(data$lpib,main="AC de LPIB")
ggAcf(data$lpib,main="AC de LPIB", lag.max = 60)
ggPacf(data$lpib, main="PAC de LPIB", lag.max = 80)
...
# Como primer paso, se estima una regresión con una constante y tendencia. 
# Se selecciona
...
# a cero (H0: B2=delta=0).
# El valor del test estadístico tiene un valor de 4.2409. Los valores críticos 
# de 10%, 5% y 1%
# son 8.43,  6.49 y  5.47, respectivamente. Por tanto, H0 no puede ser rechazada,
# lo cual implica que el LPIB tiene una raiz unitaria
# Estos resultados son confirmados por un t ratio de -2.6718 para el rezago de 
# la variable endógena. Este valor es inferior a los valores críticos (-3.99, -3.43, -3.13)
# con lo cual no es posible rechazar H0: delta=0, es decir, existe una raiz unitaria.

# Luego, se prueba si LPIB es una caminata aleatoria con o sin intercepto 
# (H0: B1=B2=delta=0). El test
# estadístico relevante es phi2, el cual tiene un valor de 20.2828, que 
# comparado a los valores críticos (6.22,  4.75,  4.07) es mayor con lo 
# que se rechaza H0, así que la # serie LPIB es una caminata aleatoria con intercepto
...
# (-3.46, -2.88, -2.57), lo que indica que LPIB tiene raíz unitaria, no existe 
# una tendencia,# pero si un intercepto en el proceso generador de datos
