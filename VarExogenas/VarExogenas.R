...
library(tidyverse); library(dynlm); library(dLagM)
...
summary(rezagod)
#q numero de rezagos que se incluye, cambiar a q=5
...
# Modelo ADL(1,1) p rezagos en X y q rezagos en Y, mirar la formula
ADL11 <- ardlDlm(formula = GDPGrowth ~ TSpread, 
                 data = data.frame(subdata), p = 1, q = 1)
summary(ADL11)
#utilice la funcion forecast pero de dLagM (: de este paquete)
#, 4 trimestre del 2013 que no utilizamos para dar valores a x, en la vida real ? 
predict.ADl11 <- dLagM::forecast(ADL11, x = data$TSpread[225:228], 
         h = 4, interval = T, nSim = 1000)
predict.ADl11 #interval true, incluye IC
#prediccion intramuestral, lo real y la predicción
error.predict <- predict.ADl11[["forecasts"]][["Forecast"]][1] - data$GDPGrowth[225]
View(data$GDPGrowth[225])
error.predict
#ts serie de tiempo
...
# Utilizando el paquete dynlm - OTRA FORMA DE ESTIMAR ADL
...
# ARMAX #cuidado con los missings
subdata2 <- subdata |> 
  select(date, GDPGrowth, TSpread, TSpread.1) |> 
  drop_na()
#igual que arima pero con xreg las X xt y xt-1
ARMAX <- arima(subdata2$GDPGrowth, 
               order =c(1, 0, 1), 
               xreg = subdata2[,3:4])
coeftest(ARMAX)
#necesito valores futuros de X
...
ggplot(data.predictARMAX[201:216,]) + 
  ...
  scale_color_manual(name = "", values = c("Observada" = "darkblue", "Predicha" = "red")) + 
  scale_fill_manual(values = c(IC = "steelblue"), labels = c(IC = "IC 95%")) + 
  labs(x = "Trimestre", y ="", fill = "") + ggtitle("Predicción del ARMAX(1,1,1)")
#data frame para la prediccion del adl y armax
...
ggplot(data.predictADLARMAX[201:216,]) + 
  ...
  theme(legend.text = element_text(size = 10), text = element_text(size=8), legend.spacing.y = unit(-0.3, "cm"), legend.background=element_blank()) + 
  guides(shape = guide_legend(order = 2),col = guide_legend(order = 1)) + 
  labs(x = "Trimestre", y ="", fill = "") + ggtitle("Predicción ADL(1,1) vs ARMAX(1,1,1)") 
